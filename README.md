...
filtering, a Sessions screen (`js`) listing local GNU screen and tmux
sessions side by side with per-row Attach/Close actions (Attach hands
the real terminal to the session, screen `-D -r`/tmux `attach -d`, and
returns to breakthrough automatically once you detach or it ends), and
real
