...
func SettingDocs() []SettingDoc {
	...
	return []SettingDoc{
		...
		{
			Key:         "status_bar_show_username",
			Default:     strconv.FormatBool(d.StatusBarShowUsername),
			Kind:        KindBool,
			Summary:     "Show the current username in the status bar.",
			Implemented: true,
		},
		{
			Key:         "status_bar_show_mouse",
			Default:     strconv.FormatBool(d.StatusBarShowMouse),
			Kind:        KindBool,
			Summary:     "Show the \"Mouse on\"/\"Mouse off\" segment in the status bar.",
			Implemented: true,
		},
		{
			Key:         "status_bar_show_disk",
			Default:     strconv.FormatBool(d.StatusBarShowDisk),
			Kind:        KindBool,
			Summary:     "Show free/total disk space for the current directory in the status bar.",
			Implemented: true,
		},
		{
			Key:         "status_bar_show_inodes",
			Default:     strconv.FormatBool(d.StatusBarShowInodes),
			Kind:        KindBool,
			Summary:     "Show used/total inode count for the current directory in the status bar.",
			Implemented: true,
		},
		{
			Key:         "status_bar_show_kernel",
			Default:     strconv.FormatBool(d.StatusBarShowKernel),
			Kind:        KindBool,
			Summary:     "Show the running kernel version (`uname -r`) in the status bar.",
			Implemented: true,
		},
		{
			Key:         "status_bar_show_uptime",
			Default:     strconv.FormatBool(d.StatusBarShowUptime),
			Kind:        KindBool,
			Summary:     "Show system uptime in the status bar, where the platform exposes it.",
			Implemented: true,
		},
		{
			Key:         "status_bar_show_load",
			Default:     strconv.FormatBool(d.StatusBarShowLoad),
			Kind:        KindBool,
			Summary:     "Show the 1/5/15-minute load average in the status bar, where the platform exposes it.",
			Implemented: true,
		},
		{
			Key:         "show_git_status",
		...
		{
			Key:         "compress_format",
			Default:     d.CompressFormat,
			Kind:        KindEnum,
			Summary:     `Compress's own last-used archive format: "zip", "tar", "tar.gz", "tar.bz2", "tar.xz", or "tar.zst". Self-adapting.`,
			Implemented: true,
		},
		{
			Key:         "open_with_command",
			Default:     d.OpenWithCommand,
			Kind:        KindString,
			Summary:     `"Open with…"'s own last-typed command, prefilled next time. Self-adapting.`,
			Implemented: true,
		},
		{
			...
			Kind:        KindBool,
			Summary:     "Rsync's own last-used \"Copy the folder's contents in\" flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "rsync_archive",
			Default:     strconv.FormatBool(d.RsyncArchive),
			Kind:        KindBool,
			Summary:     "Rsync's own last-used \"Archive mode\" (-a) flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "rsync_compress",
			Default:     strconv.FormatBool(d.RsyncCompress),
			Kind:        KindBool,
			Summary:     "Rsync's own last-used \"Compress data in transit\" (-z) flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "rsync_delete",
			Default:     strconv.FormatBool(d.RsyncDelete),
			Kind:        KindBool,
			Summary:     "Rsync's own last-used \"Delete extraneous files\" (--delete) flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "rsync_dry_run",
			Default:     strconv.FormatBool(d.RsyncDryRun),
			Kind:        KindBool,
			Summary:     "Rsync's own last-used \"Dry run\" (-n) flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "sed_regex",
			Default:     strconv.FormatBool(d.SedRegex),
			Kind:        KindBool,
			Summary:     "Sed Replace's own last-used \"Regex\" flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "sed_extended_regex",
			Default:     strconv.FormatBool(d.SedExtendedRegex),
			Kind:        KindBool,
			Summary:     "Sed Replace's own last-used \"Extended regex\" (-E) flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "sed_case_insensitive",
			Default:     strconv.FormatBool(d.SedCaseInsensitive),
			Kind:        KindBool,
			Summary:     "Sed Replace's own last-used \"Case-insensitive\" flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "sed_global",
			Default:     strconv.FormatBool(d.SedGlobal),
			Kind:        KindBool,
			Summary:     "Sed Replace's own last-used \"Replace all matches per line\" flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "sed_backup",
			Default:     strconv.FormatBool(d.SedBackup),
			Kind:        KindBool,
			Summary:     "Sed Replace's own last-used \"Keep a .bak backup\" flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "search_engine",
			Default:     d.SearchEngine,
			Kind:        KindEnum,
			Summary:     `Search's own last-used Engine: "find" or "locate". Self-adapting.`,
			Implemented: true,
		},
		{
			Key:         "search_shell_patterns",
			Default:     strconv.FormatBool(d.SearchShellPatterns),
			Kind:        KindBool,
			Summary:     "Search's own last-used \"Using shell patterns\" flag (Filename match mode). Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "search_case_sensitive",
			Default:     strconv.FormatBool(d.SearchCaseSensitive),
			Kind:        KindBool,
			Summary:     "Search's own last-used \"Case sensitive\" flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "search_skip_hidden",
			Default:     strconv.FormatBool(d.SearchSkipHidden),
			Kind:        KindBool,
			Summary:     "Search's own last-used \"Skip hidden\" flag. Self-adapting.",
			Implemented: true,
		},
		{
			Key:         "log_level",
		...
		},
		{
			...
			Default:     d.FirewallDefaultDirection,
			Kind:        KindEnum,
			Summary:     `The "Add rule" form's own starting Direction: "in" or "out". Not self-adapting — see its own doc comment.`,
			Implemented: true,
		},
		{
			Key:         "firewall_default_action",
			Default:     d.FirewallDefaultAction,
			Kind:        KindEnum,
			Summary:     `The "Add rule" form's own starting Action: "allow", "deny", or "reject". Not self-adapting.`,
			Implemented: true,
		},
		{
			Key:         "firewall_default_protocol",
			Default:     d.FirewallDefaultProtocol,
			Kind:        KindEnum,
			Summary:     `The "Add rule" form's own starting Protocol: "" (any), "tcp", or "udp". Not self-adapting.`,
			Implemented: true,
		},
	}
}
